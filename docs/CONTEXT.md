...
- **Docs/wiki sempre em dia:** `docs-sync.yml` roda a cada merge em `main` (Claude Code Action) e atualiza docs/ADRs/wiki quando o diff genuinamente exigir, commitando em `develop` (nunca em `main`). Requer secret `ANTHROPIC_API_KEY`/`CLAUDE_CODE_OAUTH_TOKEN` — ver CONTRIBUTING.md.
- **Estrutura:**
