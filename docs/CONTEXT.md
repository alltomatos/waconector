...
- **F2 — Largura** 🔶 (2026-07-11): adapters uazapi, Z-API e Wuzapi implementados e auditados adversarialmente (mesmo rigor da F1). `v0.2.0` publicado no npm. `reply/quote` já estava coberto (campo `quotedId`, ver ADR-0008). `messages.sendReaction` adicionado ao core e retrofitado nos 5 adapters existentes (ADR-0008), cada um mapeado para a particularidade real do provider de remover reação (sentinel `"remove"` no Evolution GO/Wuzapi, endpoint dedicado na Z-API, emoji vazio no WAHA/uazapi). Capability `groups.*` iniciada (ADR-0009): núcleo + participantes (`create`/`getInfo`/`list`/`add`/`remove`/`promote`/`demoteParticipants`) implementado nos 5 adapters; configurações (subject/description/picture) e convites+saída (invite link/join/leave) planejados como PRs seguintes; webhooks de grupo (`GroupUpdateEvent`) pendentes de validação contra instância real. Pendente: contatos (capability grande, tratada como incremento próprio).
