...
| `messages.forward` | `POST .../messages/forward` | Body `{to, text}` → `{message_id}`. **Escopo: só texto** (confirmado em `internal/session/actions.go`'s `buildForwardedText` — o servidor ainda não guarda o conteúdo original da mensagem a encaminhar, então recebe o texto pronto, marcado `IsForwarded`). |
