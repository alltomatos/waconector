...
- e25287d: Namespace novo `business.*` (`getProfile`/`updateProfile`) — ver ADR-0018. `WaAdapter.business?`
...
- 33a6bb2: Namespace novo `calls.*` (`make`/`reject`) — ver ADR-0019. `WaAdapter.calls?` inteiramente
...
- 8a1cbdb: Namespace novo `channels.*` (`list`/`create`/`getInfo`/`delete`/`follow`/`unfollow`) — ver
...
- c69aafb: Namespace novo `labels.*` (`list`/`create`/`update`/`delete`/`addToChat`/`removeFromChat`) — ver
...
- b202f9a: Namespace novo `presence.*` (`setTyping`/`set`/`subscribe`) — ver ADR-0015. `WaAdapter.presence?`
...
- 8afa578: Capabilities novas `messages.sendLocation`, `messages.sendContactCard` e `messages.sendPoll` — ver
